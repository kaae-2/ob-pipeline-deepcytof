...
class DeepCyTOFRunner:
    ...
    def train(self, x_path, y_path):
        print(f"DEBUG: Loading training data...", flush=True)
        ...
        print("DEBUG: Training AutoEncoder (DAE)... This may take a few minutes.", flush=True)
        self.dae_model = dae.trainDAE(target, self.output_dir, 0, np.array([0]), 
                                     None, 'CSV', 0.8, True, False, str(self.dataset_name))
        
        print("DEBUG: Denoising target data...", flush=True)
        ...
        print("DEBUG: Training Feedforward Classifier...", flush=True)
